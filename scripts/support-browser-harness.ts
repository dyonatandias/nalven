...
const assets = await build({
  entryPoints: { "support-app": "tests/fixtures/support-browser-app.tsx", "portal-shell": "tests/fixtures/portal-shell-browser-app.tsx" },
  bundle: true, write: false, outdir: "/tmp/nalven-support-browser-assets", jsx: "automatic", platform: "browser",
  conditions: ["browser", "import", "style"], define: { "process.env.NODE_ENV": '"development"' },
  plugins: [{ name: "support-next-adapters", setup(plugin) {
    plugin.onResolve({ filter: /^next\/(link|navigation|image)$/ }, args => ({ path: args.path, namespace: "support-next" }));
    plugin.onLoad({ filter: /.*/, namespace: "support-next" }, args => ({ loader: "tsx", resolveDir: process.cwd(), contents:
...
const server = createServer((request, response) => {
  const path = new URL(request.url || "/", "http://127.0.0.1:4181").pathname;
  response.setHeader("cache-control", "no-store"); response.setHeader("referrer-policy", "no-referrer");
  if (path.startsWith("/api/")) { response.writeHead(503, { "content-type": "application/json" }); response.end(JSON.stringify({ error: "Support API must be mocked explicitly by browser tests." })); return; }
  ...
  const entry = path === "/__portal-shell" ? "portal-shell" : "support-app";
...
server.listen(4181, "127.0.0.1", () => console.log("Support browser harness: http://127.0.0.1:4181"));
