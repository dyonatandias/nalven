...
export async function signupConfiguration(){
 ...
 const config=signup?.value as {billingPlanCodes?:Record<string,string>;paymentMethods?:string[];dueDay?:number}|null;
 const platform=saas?.value as {domain?:string;trialDays?:number}|null;
 if(!config?.billingPlanCodes||!Array.isArray(config.paymentMethods)||!config.paymentMethods.length||config.paymentMethods.some(method=>!['pix','boleto'].includes(method))||!Number.isInteger(config.dueDay)||config.dueDay!<1||config.dueDay!>28||!platform?.domain||!/^[a-z0-9.-]+$/.test(platform.domain)||!Number.isInteger(platform.trialDays)||platform.trialDays!<0||platform.trialDays!>90)throw new HttpSecurityError("Contratação indisponível: configuração comercial incompleta.",503);
 return {billingPlanCodes:config.billingPlanCodes,paymentMethods:config.paymentMethods,dueDay:config.dueDay!,trialDays:platform.trialDays!,origin:`https://${platform.domain}`};
}
