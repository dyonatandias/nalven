-- SHA-256 commitments are deliberately persisted throughout the T2 graph.
-- Inspecting their random hexadecimal contents as natural-language evidence
-- creates false positives (for example, a digest containing "e2e"). Remove
-- only exact quoted 64-hex commitments before applying the existing DLP rules.
...
AS $function$
 WITH sanitized AS (
   SELECT pg_catalog.regexp_replace(p_canonical, '"[0-9a-f]{64}"', '""', 'g') AS value
 )
 SELECT NOT (
