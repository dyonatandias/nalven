...
test("configuração valida senha, planos e versões preservando campos internos", async () => {
  const mocks: Record<string, string> = {
    ...
    "@/db/control": `import {state} from 'test:state';export const controlDb={plan:{findMany:async()=>[{id:'public',active:true,visibility:'public'}]},systemSetting:{findMany:async()=>[{key:'saas',updatedAt:new Date('2026-09-09T00:00:00Z'),value:{tenantIsolation:'database-per-tenant'}},{key:'signup',updatedAt:new Date('2026-09-09T00:00:00Z'),value:{internalMarker:'preserve'}}],upsert:async args=>state.writes.push(args)},auditLog:{create:async args=>state.audit.push(args)},$executeRaw:async()=>0,$transaction:async callback=>callback(controlDb)};`,
  };
  ...
  const body = { name: "Nalven", domain: "example.test", trialDays: 14, dueDay: 10, paymentMethods: ["pix"], billingPlanCodes: { public: "billing-public" }, versions: { saas: "2026-09-09T00:00:00.000Z", signup: "2026-09-09T00:00:00.000Z" }, currentPassword: "fixture-confirmation" };
  ...
  for (const patch of [{ domain: "https://example.test" }, { trialDays: 91 }, { dueDay: 29 }, { paymentMethods: [] }, { billingPlanCodes: {} }]) assert.equal((await POST(request({ ...body, ...patch }))).status, 400);
